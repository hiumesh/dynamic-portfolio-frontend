...
import { getAll } from "@/services/api/portfolio";
...
export default function ListContainer() {
  const { filters } = usePortfolioContext();
  ...
  const {
    ...
    queryKey: ["portfolios", "infinite", filters.query],
  ...
  const loadingView = (
    <>
      <PortfolioCardSkeleton /> <PortfolioCardSkeleton />
      <PortfolioCardSkeleton /> <PortfolioCardSkeleton />
      <PortfolioCardSkeleton /> <PortfolioCardSkeleton />
      <PortfolioCardSkeleton /> <PortfolioCardSkeleton />
    </>
  ...
  return (
    <MaxWidthWrapper>
      ...
      <div className="grid grid-cols-3 gap-4">
        {data?.pages
          .reduce<PortfolioListItem[]>(
            (prev, curr) => [...prev, ...(curr?.list || [])],
            []
          )
          .map((page) => (
            <PortfolioCard key={page.id} data={page} />
          ))}
