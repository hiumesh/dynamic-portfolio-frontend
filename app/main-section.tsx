...
export default function MainSection() {
  ...
  return (
    ...
    >
      <div className="fixed left-0 -z-10 top-0 h-full w-full">
        <div className="relative  h-full w-full bg-black">
          <div className="absolute bottom-0 left-0 right-0 top-0 bg-[linear-gradient(to_right,#4f4f4f2e_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:14px_24px]"></div>
          <div className="absolute left-[50%] -translate-x-1/2 right-0 top-[-10%] h-full w-full bg-[radial-gradient(circle_800px_at_50%_800px,var(--hero-bg-gradient-color),#000)]"></div>
        </div>
      </div>

      <section className="mx-auto  max-w-screen-xl min-h-screen flex flex-col items-center p-10 pt-24 space-y-10">
        ...
        <div className="w-full flex gap-3 text-white">
          {items.map((item) => (
            <div
              key={item.title}
              className="group text-center flex-1 bg-gray-800/40 backdrop-blur-md rounded-md first:rounded-l-3xl last:rounded-r-3xl p-6 space-y-4 hover:scale-x-105 transition-all"
